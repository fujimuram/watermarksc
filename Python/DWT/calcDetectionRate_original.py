...
def calc_failrate(img_original_pre, ws, N, Q, BLOCK_SIZE, target, modulo):
    ...
    for i in range(len(ws)):
        ...
        img_embs[i] , LM[i] = cmdb.embed(img_original, ws[i], emb_block)
        psnr[i] = cv2.PSNR(img_original, img_embs[i])
        ...
        extract_imgs[i] = cmdb.extract(img_embs[i], LM[i])
    ...
    for i in range(len(img_embs)):  
        ...
        w_mean_ext = cmdb.extract(img_mean, LM[i])
...
def main():
    ...
    for q in range(1,Q):
        print(q, (q/Q) * 100, "%")
        tmp_fail_rate, tmp_fail_block_rate, tmp_fail_white_rate, tmp_psnr = calc_failrate(img_original_pre, ws, N, q, BLOCK_SIZE, target, modulo)
    ...
    np.savetxt("BlackDetectionRate_original.csv", fail_block_rate, delimiter=",", fmt="%d")
    
    f = open('PSNR_Original.csv', 'w')
    writer = csv.writer(f)
