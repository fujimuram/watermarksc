...
block_number =  img_original.shape // cmdb.need_size // watermarks[1].shape
...
for i in range(block_number[0] * block_number[1]):
    ...
    img_embs[i] = cmdb.embed(img_original, ws[i_shift], emb_block)
...
for i in range(len(img_embs)):  
    ...
    extract_w, _S = cmdb.extract(img_mean, diff)
