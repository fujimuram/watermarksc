...
def calc_failrate(img_original_pre, ws, N, Q, BLOCK_SIZE, target, modulo):
    ...
    for i in range(len(ws)):
        ...
        img_embs[i] = cmdb.embed(img_original, ws[i], emb_block)
    ...
    for i in range(len(img_embs)):  
        ...
        w_mean_ext, _S = cmdb.extract(img_mean, diff)
...
def main():
    ...
    np.savetxt("BlackDetectionRate_myemb2.csv", fail_block_rate, delimiter=",", fmt="%d")
    
    f = open('PSNR_myemb2.csv', 'w')
    writer = csv.writer(f)
