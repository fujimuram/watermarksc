...
from my_packege.my_class.ClassMultipleDWTandBlock_bug import ClassMultipleDWTandBlock as CMDBB
...
for i in range(block_number[0] * block_number[1]):
    ...
    plt.subplot(4,4,i+1)
    plt.imshow(cmdb.extract(embed_img, LM[i]), vmin=0, vmax=255, cmap = 'gray')
    #print(i, i_shift)
...
for i in range(len(embed_imgs)):  
    ...
    for j in range(img_mean_blocks.shape[0] * img_mean_blocks.shape[1]):
        ...
        #print("j = ",j, ", ", row, col)
        extract_w[row,col] = cmdb.extract(img_mean_blocks[row,col], LM[j])
